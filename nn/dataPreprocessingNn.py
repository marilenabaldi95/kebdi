...
qa_json_train_path = 'data/qa/train_qa.json'
qa_json_test_path = 'data/qa/test_qa.json'
plots_path = 'data/plot'
data_path = 'data'

TRAIN = False #settare a False se si vuole elaborare il test set

#nltk.download() #al primo utilizzo decommentare e scaricare tutto
...
#fa anche il padding delle frasi della trama (per poter dare alla rete neurale un ingresso costante)
#salva i vettori risultanti (trame, domande, risposte) in file numpy (.npy)
def elaborate(padding=100, custom_data=None):
    if custom_data is not None:
        data = custom_data
    else:
        if TRAIN is True:
            data = elaborate_data(qa_json_train_path, plots_path)
        else:
            data = elaborate_data(qa_json_test_path, plots_path)
    ...
    if custom_data is not None:
        ...
        if TRAIN is True:
            np.save(data_path + '/train_phrases.npy', train_phrases)
            np.save(data_path + '/train_questions.npy', train_questions)
            np.save(data_path + '/train_answers.npy', train_answers)
        else:
...
#funzione utilizzata dalla GUI della rete neurale per elaborare (come nella funzione elaborate() una sola coppia [trama, domanda])
...
def userelaborate(data, padding=100):
    train_phrases = []
    train_questions = []
    ...
    return [train_phrases, train_questions]
